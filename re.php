<?php
include ("config.php");
?>

<html>
<head>
<meta http-equiv="Content-Type" content="text/html; charset=utf-8" />
<title>Untitled Document</title>
<link href="css/re.css" rel="stylesheet" type="text/css" />
<link href="https://fonts.googleapis.com/css?family=Play" rel="stylesheet">
    <style>
        button
        {
            border: none;
            background: white;
            color: black;
            font-size: 16px;
            line-height: 25px;
            padding: 10px 0;
            border-radius: 12px;
            cursor: pointer;
            width: 180px;
            margin-left: 30px;
        }
        button:hover
        {
            background: black;
            color: white;
        }
#msg {
       visibility: hidden;
    min-width: 250px;
    
    background-color:yellow;
    color: #000;
    text-align: center;
    border-radius: 2px;
    padding: 16px;
    position: fixed;
    z-index: 1;
    right: 30%;
    top: 30px;
    font-size: 17px;
	margin-right:50px;
}

#msg.show {
    visibility: visible;
    -webkit-animation: fadein 0.5s, fadeout 0.5s 2.5s;
    animation: fadein 0.5s, fadeout 0.5s 2.5s;
}

@-webkit-keyframes fadein {
    from {top: 0; opacity: 0;} 
    to {top: 30px; opacity: 1;}
}

@keyframes fadein {
    from {top: 0; opacity: 0;}
    to {top: 30px; opacity: 1;}
}

@-webkit-keyframes fadeout {
    from {top: 30px; opacity: 1;} 
    to {top: 0; opacity: 0;}
}

@keyframes fadeout {
    from {top: 30px; opacity: 1;}
    to {top: 0; opacity: 0;}
}
</style>
</head>

<body>
<div class="reset">

<form method="post">
<h2 align="center" style="color:#fff;">Reset password</h2>
<!--<input type="password" name="Name"  disabled /><br /><br />-->
<input type="password" name="password" placeholder="New password" /><br /><br />
<input type="password" name="password1" placeholder="Confirm new password" /><br /><br />
<button type="submit" name="submit4" onclick="myFunction()">Save</button>
    <br /><br />
<a href="logn.php" style="text-decoration:none;">Go back to Home page</a><br /><br />
<div id="msg">Your password changed successfully!!</div>

<script>
function myFunction() {
    var x = document.getElementById("msg");
    x.className = "show";
    setTimeout(function(){ x.className = x.className.replace("show", ""); }, 5000);
}
</script>

    <?php
    session_start();
    include ('config.php');
    $email=$_SESSION['mail'] ;
    if(isset($_POST['submit4'])){

        $password = mysqli_real_escape_string($connection,$_POST['password1']);

        $query1=mysqli_query($connection,"update admininfo SET PassWord='$password' where Email='$email'");
        if(!$query1){
            die("error".mysqli_error($connection));
        }
        else{
            session_destroy();
        }
    }
    ?>
</form>
</div>
</body>
</html>
